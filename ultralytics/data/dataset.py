...
class YOLODataset(BaseDataset):
                        ...
                        x["labels"].append(
                            {
                                ...
                                "shape": shape,
                                "cls": lb[:, 0:1],  # n, 1
                                "bboxes": lb[:, 1:] if len(lb[0]) != 6 else lb[:, 2:],  # n,
                                "extra": np.ones_like(lb[:, 1:2]) if len(lb[0]) != 6 else lb[:, 1:2],  # 图像熵量化检测难度，如果存在直接引入，不存在则都设置为1（即不存在检测难度差异）
                                "segments": segments,
                                "keypoints": keypoint,
                                "normalized": True,
                                "bbox_format": "xywh",
                         ...
                         print(f'检查该路径图像对应标签文件是否为空{im_file}')
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        """
        new_batch: dict[str | Any, list[Any] | Tensor | Any] = {}
        ...
        for i, k in enumerate(keys):
            ...
            if k in {"masks", "keypoints", "bboxes", "cls",  "extra", "segments", "obb"}:
