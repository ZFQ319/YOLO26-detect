...
class BboxLoss(nn.Module):
    """Loss for axis-aligned bounding boxes."""

    def __init__(self, reg_max: int = 16):
        super().__init__()
        self.reg_max = reg_max  # DFL离散回归的最大分桶数（每个边预测reg_max个概率）
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None  # 如果reg_max>1则启用DFL，否则用L1

    def forward(
        self,
        pred_dist: torch.Tensor,      # 预测的分布 (B, N, 4*reg_max) 或 (B, N, 4)
        pred_bboxes: torch.Tensor,    # 预测框 (B, N, 4)
        anchor_points: torch.Tensor,  # anchor点 (N, 2)
        target_bboxes: torch.Tensor,  # GT框 (B, N, 4)
        target_scores: torch.Tensor,  # 分类得分 (B, N, C)
        gt_loss_a: torch.Tensor | None,  # 自定义权重 (B, N, 1)
        target_scores_sum: torch.Tensor, # 所有正样本权重和（标量）
        fg_mask: torch.Tensor,        # 前景mask (B, N)
        imgsz: torch.Tensor,          # 图像尺寸 (2,) -> [H, W]
        stride: torch.Tensor,         # stride（每个点对应缩放）
    ):
        # ====== 取前景样本 ======
        pred_bboxes_fg = pred_bboxes[fg_mask]        # 选出正样本预测框 (N_fg, 4)
        target_bboxes_fg = target_bboxes[fg_mask]    # 对应GT框 (N_fg, 4)
        ...
        if gt_loss_a is not None:
            loss_iou = gt_loss_a[fg_mask] * loss_iou
            # 额外乘你自定义的动态权重
        ...
        if self.dfl_loss:
            ...
            # 将bbox转换成ltrb距离（离散分布标签） (B, N, 4)

            pred_dist_fg = pred_dist[fg_mask].view(-1, self.reg_max)
            # reshape成 (N_fg*4, reg_max)，每条边一个分布
            ...
            target_ltrb = self._normalize(target_ltrb, imgsz, stride)
            # GT归一化到[0,1]

            pred_dist = self._normalize(pred_dist, imgsz, stride)
            # 预测也归一化
        ...
        if gt_loss_a is not None:
            loss_dfl = gt_loss_a[fg_mask] * loss_dfl
            # 再加一次你的动态权重
    ...
    @staticmethod
    def _normalize(x, imgsz, stride):
        """Normalize bbox distances to [0,1]"""
        x = x * stride                # 从feature map尺度映射回原图尺度
        x[..., 0::2] /= imgsz[1]      # x方向（left/right）除以宽
        x[..., 1::2] /= imgsz[0]      # y方向（top/bottom）除以高
        return x


class RotatedBboxLoss(BboxLoss):
    """Loss for rotated bounding boxes."""

    def forward(
        self,
        pred_dist: torch.Tensor,
        pred_bboxes: torch.Tensor,    # (B, N, 5) -> x,y,w,h,angle
        anchor_points: torch.Tensor,
        target_bboxes: torch.Tensor,  # (B, N, 5)
        gt_loss_a: torch.Tensor | None,
        ...
        # ====== 前景筛选 ======
        pred_bboxes_fg = pred_bboxes[fg_mask]        # (N_fg, 5)
        target_bboxes_fg = target_bboxes[fg_mask]    # (N_fg, 5)
        ...
        if self.dfl_loss:
            target_ltrb = rbox2dist(
                target_bboxes[..., :4],   # xywh
                anchor_points,
                target_bboxes[..., 4:5],  # angle
...
class v8DetectionLoss:
    ...
    def bbox_decode(self, anchor_points: torch.Tensor, pred_dist: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_dfl:
            ...
            # pred_dist = pred_dist.view(b, a, c // 4, 4).transpose(2,3).softmax(3).matmul(self.proj.type(
            # pred_dist.dtype))
            # pred_dist = (pred_dist.view(b, a, c // 4, 4).softmax(2) * self.proj.type(
            # pred_dist.dtype).view(1, 1, -1, 1)).sum(2)
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        targets = torch.cat((batch["batch_idx"].view(-1, 1), batch["cls"].view(-1, 1), batch["bboxes"],
                             batch["extra"].view(-1, 1)), 1)
        targets = self.preprocess(targets.to(self.device), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
        gt_labels, gt_bboxes = targets[..., 0:1], targets[..., 1:]  # cls, xyxy or axyxy

        mask_gt = gt_bboxes[..., :-1].sum(2, keepdim=True).gt_(0.0)

        # Pboxes
        pred_bboxes = self.bbox_decode(anchor_points, pred_distri)  # xyxy, (b, h*w, 4)

        _, target_bboxes_loss_a, target_scores, fg_mask, target_gt_idx = self.assigner(
        ...
        if target_bboxes_loss_a.shape[2] == 5:
            target_bboxes, gt_loss_a = target_bboxes_loss_a.split((4, 1), 2)
        else:
            gt_loss_a = None
            target_bboxes = target_bboxes_loss_a
...
class v8OBBLoss(v8DetectionLoss):
    ...
    def loss(self, preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        try:
            ...
            targets = torch.cat((batch_idx, batch["cls"].view(-1, 1), batch["bboxes"].view(-1, 5),
                                 batch["extra"].view(-1, 1)), 1)
            rw, rh = targets[:, 4] * float(imgsz[1]), targets[:, 5] * float(imgsz[0])
            targets = targets[(rw >= 2) & (rh >= 2)]  # filter rboxes of tiny size to stabilize training
            targets = self.preprocess(targets.to(self.device), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
            gt_labels, gt_bboxes = targets[..., 0:1], targets[..., 1:]  # cls,
            mask_gt = gt_bboxes[..., :-1].sum(2, keepdim=True).gt_(0.0)
        ...
        # Pboxes
        pred_bboxes = self.bbox_decode(anchor_points, pred_distri, pred_angle)  # xy wh range, (b, h*w, 5)

        bboxes_for_assigner = pred_bboxes.clone().detach()
        # Only the first four elements need to be scaled
        bboxes_for_assigner[..., :4] *= stride_tensor
        _, target_bboxes_a, target_scores, fg_mask, _ = self.assigner(
            pred_scores.detach().sigmoid(),
        ...
        target_bboxes, gt_loss_a = target_bboxes_a[..., :-1], target_bboxes_a[..., -1:]
